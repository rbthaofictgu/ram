 ...
 class Api_Ram
{
	...
	protected $appcfg_max_periodos_anterior_cobrar;

	public function __construct($db,$appcfg_Dominio,$appcfg_Dominio_Corto,$appcfg_Dominio_Raiz,$appcfg_smtp_server,$appcfg_smtp_port,$appcfg_smtp_user,$appcfg_smtp_password,$appcfg_estado_inicial,$appcfg_estado_inicial_descripcion,$appcfg_placas,$appcfg_max_periodos_anterior_cobrar)
	{
		$this->setDB(        $db,
		         $appcfg_Dominio,
		...
		$appcfg_placas,
									$appcfg_max_periodos_anterior_cobrar);
		...
		if (isset($_POST["action"])) {
			if ($_POST["action"] == "get-apoderado" && isset($_POST["idApoderado"])) {
			...
			} else if ($_POST["action"] == "get-concesion-expediente" && isset($_POST["idConcesion"])) {
				$this->getConcesionExpediente();
			} else if ($_POST["action"] == "get-datosporomision") {
				$this->getDatosPorOmision();
			} else if ($_POST["action"] == "get-datosporomisionpaginacion") {
				$this->getDatosPorOmisionPaginacion();				
				...
				$this->updatePreforma();
			} else if ($_POST["action"] == "save-expediente" and $_POST["modalidadDeEntrada"] == "I") {
				$this->saveExpediente();
			} else if ($_POST["action"] == "save-expediente" and $_POST["modalidadDeEntrada"] == "U") {
	...
	protected function setDB($db,$appcfg_Dominio,$appcfg_Dominio_Corto,$appcfg_Dominio_Raiz,$appcfg_smtp_server,$appcfg_smtp_port,$appcfg_smtp_user,$appcfg_smtp_password,$appcfg_estado_inicial,$appcfg_estado_inicial_descripcion,$appcfg_placas,$appcfg_max_periodos_anterior_cobrar): void	{
		$this->db = $db;
		...
		$this->appcfg_max_periodos_anterior_cobrar = $appcfg_max_periodos_anterior_cobrar;
	}
	...
	protected function saveIhttDbExpedienteMovimiento($RAM) : string {
		...
		$query = "INSERT INTO [IHTT_DB].[dbo].[TB_Expediente_Movimiento] 
				...
				ID_Area_Iba,
				ID_Actividad,
				FechaRecibe,FechaEnvia,
				CodigoEstadoMovimiento, 
			...
			WHERE NOT EXISTS (
				...
				WHERE ID_Solicitud = :ID_Solicitud1 AND ID_Actividad = 'ACT-01'
			);";
		$parametros = array(":ID_Solicitud" =>$RAM, ":ID_Solicitud1" =>$RAM, ":ID_Area_Actual" =>$ID_AREA_VENTANILLA, ":SistemaUsuario"=>$_SESSION["user_name"]);
	...
	protected function cerrarPreforma() {
		...
		if (!isset($respuestaPDFAvisodeCobroVentanillaApi['error']) and !isset($respuestaPDFAvisodeCobroVentanillaApi['ERROR'])) {
	...
	protected function deleteConcesionesExpediente()
	{
		...
		$this->db->beginTransaction();
		foreach ($_POST["idConcesiones"] as $Concesion) {
			$query = "DELETE FROM [IHTT_DB].[dbo].[TB_Expediente_X_Tipo_Tramite] where ID_Solicitud = :ID_Solicitud and (Certificado_Operacion = :Certificado_Operacion or N_Permiso_Especial = :N_Permiso_Especial or Permiso_Explotacion = :Permiso_Explotacion)";
			$p = array(":ID_Solicitud" => $_POST["RAM"], ":Certificado_Operacion" => $Concesion, ":N_Permiso_Especial" => $Concesion, ":Permiso_Explotacion" => $Concesion);
			$return = $this->delete($query, $p);
			if ($return == false) {
				break;
			}
			$query = "DELETE FROM [IHTT_DB].[dbo].[TB_Solicitud_Vehiculo_Actual] where ID_Solicitud = :ID_Solicitud and (Numero_Certificado = :Numero_Certificado or Numero_PermisoEspecial = :Numero_PermisoEspecial or Numero_Explotacion = :Numero_Explotacion)";
			$p = array(":ID_Solicitud" => $_POST["RAM"], ":Numero_Certificado" => $Concesion, ":Numero_PermisoEspecial" => $Concesion, ":Numero_Explotacion" => $Concesion);
			$return = $this->delete($query, $p);
			if ($return == false) {
				break;
			}
	...
	//*rbthaofic@gmail.com 2024/12/05 Delete Tramite Preforma */
	//***********************************************************************************************************************/
	protected function deleteTramiteExpediente()
	{
		$_POST["idTramite"] = json_decode($_POST["idTramite"], true);
		...
		$this->db->beginTransaction();
		$query = "DELETE FROM [IHTT_DB].[dbo].[TB_Expediente_X_Tipo_Tramite] where ID = :ID";
		$p = array(":ID" => $_POST["idTramite"]);
		$return = $this->delete($query, $p);
		if ($return == false) {
			$this->db->rollBack();
			...
			if (isset($_POST["ID_Unidad1"])) {
				$query = "DELETE FROM [IHTT_DB].[dbo].[TB_Solicitud_Vehiculo_Entra] where ID = :ID";
				$p = array(":ID" => intval($_POST["ID_Unidad1"])); 
				$return = $this->delete($query, $p);
				if ($return == false) {
					$this->db->rollBack();
					echo json_encode(array("error" => 2002, "errorhead" => "ELIMINAR TRAMITE EXPEDIENTE", "errormsg" => 'INCONVENIENTES AL INTENTAR ELIMINAR LA UNIDAD ENTRANTE'));
				} else {								
					$this->db->commit();
					//$this->db->rollBack();
					echo json_encode(['Borrado'  =>  True,'Adentro'  =>  True, 'ID_Unidad1' => intval($_POST["ID_Unidad1"]), 'ID_Unidad' => intval($_POST["ID_Unidad"])]);
	...
	protected function getSolicitanteRAM()
	{
	  $q = "SELECT 
			...
			es.esEditable,
			es.esCompartible,
			ISNULL(ENC.CodigoAvisoCobro,0) AS CodigoAvisoCobro
		FROM 
		...
		if (!isset($_POST["echo"])) {
			...
			echo json_encode($this->select($q, array(':ID_Formulario_Solicitud' => $_POST["RAM"],':Usuario_Creacion' => $_SESSION["user_name"],':Usuario_Acepta' => $_SESSION["user_name"],':Consulta' => intval($_POST["Consulta"]),':Usuario_Comparte' => $_SESSION["user_name"])));
		}
	}
	...
	protected function getSolicitanteEXP()
	{
	  $q = "SELECT  us.ID_Empleado,soli.Usuario_Creacion,soli.Usuario_Acepta,soli.Codigo_Usuario_Acepta,ISNULL(ENC.CodigoAvisoCobro,0) AS CodigoAvisoCobro,ISNULL(ENC.AvisoCobroEstado,'') AS AvisoCobroEstado,
			...
			FROM [IHTT_PREFORMA].[dbo].[TB_Solicitante] soli left outer join [IHTT_USUARIOS].[dbo].[TB_Usuarios] us on soli.Usuario_Creacion = us.Usuario_Nombre,
			[IHTT_DB].[dbo].[TB_Solicitante] sol,[IHTT_DB].[dbo].[TB_Expedientes] ex
			left outer join [IHTT_GDL].[dbo].[TB_AutoAdmision] aa on ex.ID_Solicitud = aa.ID_Solicitud
			left outer join [IHTT_GDL].[dbo].[TB_Resolucion] res on ex.ID_Solicitud = res.ID_Solicitud
			left outer join [IHTT_Webservice].[dbo].[TB_AvisoCobroEnc] ENC on ex.ID_Solicitud = ENC.ID_Solicitud  and ENC.AvisoCobroEstado != 3,
			...
			WHERE soli.ID_Formulario_Solicitud = :ID_Formulario_Solicitud and 
			soli.[ID_Formulario_Solicitud] = ex.Preforma and
			soli.[RTN_Solicitante] = sol.RTNSolicitante and
			soli.[RTN_Solicitante] = sol.RTNSolicitante and
			...
			soli.Estado_Formulario = eu.ID_Estado and eu.estado = 1 and
			sol.[CodigoSolicitanteTipo] = st.[ID_Tipo_Solicitante] and 
			(soli.Usuario_Creacion = :Usuario_Creacion or soli.Usuario_Acepta = :Usuario_Acepta or 0 = :Consulta or
			exists(select Usuario_Comparte from [IHTT_RENOVACIONES_AUTOMATICAS].[dbo].[TB_RAM_A_COMPARTIR] SH 
			where soli.ID_Formulario_Solicitud = SH.ID_Formulario_Solicitud and SH.Usuario_Comparte = :Usuario_Comparte and soli.Estado_Formulario = SH.Estado_Formulario))";
		if (!isset($_POST["echo"])) {
			return $this->select($q, array(':ID_Formulario_Solicitud' => $_POST["RAM"],':Usuario_Creacion' => $_SESSION["user_name"],':Usuario_Acepta' => $_SESSION["user_name"],':Consulta' => intval($_POST["Consulta"]),':Usuario_Comparte' => $_SESSION["user_name"]));
		} else {
			echo json_encode($this->select($q, array(':ID_Formulario_Solicitud' => $_POST["RAM"],':Usuario_Creacion' => $_SESSION["user_name"],':Usuario_Acepta' => $_SESSION["user_name"],':Consulta' => intval($_POST["Consulta"]),':Usuario_Comparte' => $_SESSION["user_name"])));
		...
		protected function getTramitesRAMTotales()
	{
		$q = "SELECT 
			isnull(count(*),0) as total_tramites
			FROM 
			[IHTT_Preforma].[dbo].[TB_Solicitud] sol
			WHERE sol.ID_Formulario_Solicitud = :ID_Formulario_Solicitud";
	...
	protected function getTramitesRAM()
	{
		...
		$q = "SELECT 
				...
				CASE 
					WHEN CO.[Clase Servicio] IN ('STPC', 'STPP') THEN CO.CertificadoEncriptado
					WHEN CO.[Clase Servicio] = 'STEP' THEN CO.PermisoEspecialEncriptado
					ELSE CO.PermisoEspecialEncriptado
				END AS CertificadoEncriptado,		
				CO.PermisoEspecialEncriptado,		
				CASE 
					WHEN CO.[Clase Servicio] IN ('STPC', 'STPP') THEN CO.Permiso_Explotacion_Encriptado
					WHEN CO.[Clase Servicio] = 'STEP' THEN 'NO TRAE'
					ELSE 'NO TRAE'
				END AS Permiso_Explotacion_Encriptado,				
				CO.[Clase Servicio],
				CASE 
			...
			OFFSET CAST(:offset as int) ROWS
        	FETCH NEXT CAST(:pageSize as int) ROWS ONLY;";
		$rows = $this->select($q, array(':ID_Formulario_Solicitud' => $_POST["RAM"],':offset' => $offset,':pageSize' => $pageSize));
		$max = count($rows);
		for ($i=0; $i<$max; $i++) {
			...
			while($Permiso_Explotacion_Encriptado != $rows[$i]["Permiso_Explotacion_Encriptado"]){        
				...
				while($CertificadoEncriptado != $rows[$i]["CertificadoEncriptado"]){        
					...
					if ($rows[$i]["ID_CHECK"] === 'IHTTTRA-02_CLATRA-01_R_PE' || $rows[$i]["ID_CHECK"] === 'IHTTTRA-02_CLATRA-02_R_CO' || $rows[$i]["ID_CHECK"] === 'IHTTTRA-02_CLATRA-03_R_PS') {
...
protected function getTramitesEXPTotales()
	{
		$q = "SELECT isnull(count(*),0) as total_tramites
			FROM
				[IHTT_DB].[dbo].[TB_Expedientes] Ex 
			JOIN
				[IHTT_DB].[dbo].[TB_Expediente_X_Tipo_Tramite] Tra ON tra.ID_Solicitud = ex.ID_Solicitud
			WHERE 
				ex.Preforma = :ID_Solicitud;";
	...
	protected function getTramitesEXP()
	{
		...
		$q = "SELECT 
				...
				ISNULL(CO.N_Certificado,'SIN-CONCESION') AS N_Certificado,
				ISNULL(CO.[Fecha Vencimiento Certificado],SYSDATETIME()) AS Fecha_Expiracion,
				ISNULL(CO.N_Permiso_Explotacion,'') AS N_Permiso_Explotacion,
				ISNULL(CO.[Fecha Vencimiento Permiso],SYSDATETIME()) AS Fecha_Expiracion_Explotacion
			FROM
				[IHTT_DB].[dbo].[TB_Expedientes] Ex 
			JOIN
				[IHTT_DB].[dbo].[TB_Expediente_X_Tipo_Tramite] Tra ON tra.ID_Solicitud = ex.ID_Solicitud
			...
			OFFSET CAST(:offset as int) ROWS
        	FETCH NEXT CAST(:pageSize as int) ROWS ONLY;";
		$rows = $this->select($q, array(':ID_Solicitud' => $_POST["RAM"],':offset' => $offset,':pageSize' => $pageSize));
		$max = 0;
		if (is_array($rows)) {
			$max = count($rows);
		}
	...
	protected function getUnidadesTotales()
	{
		$q = "WITH SolicitudPrimero AS (
			SELECT distinct
			sol.ID_Formulario_Solicitud,
			CASE 
			WHEN ISNULL(RTRIM(sol.N_Certificado), '') <> '' THEN RTRIM(sol.N_Certificado)
			ELSE RTRIM(sol.N_Permiso_Especial)
			END AS Certificado_Operacion,
			sol.Permiso_Explotacion
			FROM [IHTT_PREFORMA].[dbo].[TB_Solicitud] sol where sol.ID_Formulario_Solicitud = :ID_Formulario_Solicitud1
		)
		SELECT 
			isnull(count(*),0) as total_unidades
		FROM 
			[IHTT_PREFORMA].[dbo].[TB_Vehiculo] veh
		JOIN 
			SolicitudPrimero sol ON sol.ID_Formulario_Solicitud = veh.ID_Formulario_Solicitud and 
			((veh.Certificado_Operacion <> '' and veh.Certificado_Operacion = sol.Certificado_Operacion) or veh.Permiso_Especial <> '' and veh.Permiso_Especial = sol.Certificado_Operacion )
		WHERE 
			veh.ID_Formulario_Solicitud = :ID_Formulario_Solicitud;";
		if (!isset($_POST["echo"])) {
			return $this->select($q, array(':ID_Formulario_Solicitud' => $_POST["RAM"],':ID_Formulario_Solicitud1' => $_POST["RAM"]));
		} else {
			echo json_encode($this->select($q, array(':ID_Formulario_Solicitud' => $_POST["RAM"])));
		}
	}

	protected function getUnidades($TRAMITES)
	{
		//*****************************************************************************************************/
		//* Construir la condicion para el IN de las concesiones
		//*****************************************************************************************************/
		$c = count($TRAMITES);
		$concesiones = '(';
		for ($i = 0; $i < $c; $i++) {
			$concesiones .= "'" . $TRAMITES[$i]["N_Certificado"] . "'";
			if ($i == 0 and $TRAMITES[$i]["ID_Clase_Servicio"]) {$ID_Clase_Servicio = $TRAMITES[$i]["ID_Clase_Servicio"];}
		}
		$concesiones .=	 ')';
		//*****************************************************************************************************/
		//* Dependiendo si son certificados o permisos especiales se arma la condicion
		//*****************************************************************************************************/
		if ($ID_Clase_Servicio == 'STPC' or $ID_Clase_Servicio == 'STPP') {
			$condicion = "N_Certificado IN " . $concesiones;
		} else {
			$condicion = "N_Permiso_Especial IN " . $concesiones;
		}
		//*****************************************************************************************************/
		//* Armando SQL de Unidades
		//*****************************************************************************************************/
		$q = "WITH SolicitudPrimero AS (
			SELECT distinct
			sol.ID_Formulario_Solicitud,
			...
			END AS Certificado_Operacion,
			sol.Permiso_Explotacion
			FROM [IHTT_PREFORMA].[dbo].[TB_Solicitud] sol where sol.ID_Formulario_Solicitud = :ID_Formulario_Solicitud AND ". $condicion . "
		)
		SELECT 
			CASE 
			WHEN ISNULL(RTRIM(veh.Certificado_Operacion), '') <> '' THEN RTRIM(veh.Certificado_Operacion)
			WHEN ISNULL(RTRIM(veh.Permiso_Especial), '') <> '' THEN RTRIM(veh.Permiso_Especial)
			WHEN ISNULL(RTRIM(sol.Certificado_Operacion), '') <> '' THEN RTRIM(sol.Certificado_Operacion)
			ELSE 'SIN-CONCESION'
			END AS Certificado_Operacion,
			vl.[Clase Servicio],
			CASE 
			...
			COALESCE(
				...
				AND (veh.Estado = 'ENTRA' or (veh.Estado = 'NORMAL' and LEN(LTRIM(RTRIM(ID_Placa))) < 7)),
				'false'
			) AS ID_Memo
		FROM 
			[IHTT_PREFORMA].[dbo].[TB_Vehiculo] veh
		...
		WHERE veh.ID_Formulario_Solicitud = :ID_Formulario_Solicitud1
		ORDER BY veh.Certificado_Operacion, veh.Permiso_Especial, veh.Estado DESC;";
		if (!isset($_POST["echo"])) {
			return $this->select($q, array(':ID_Formulario_Solicitud' => $_POST["RAM"],':ID_Formulario_Solicitud1' => $_POST["RAM"]), PDO::FETCH_GROUP);
		} else {
			echo json_encode($this->select($q, array(':ID_Formulario_Solicitud' => $_POST["RAM"]), PDO::FETCH_GROUP));
		}
	}

protected function getUnidadesEXPTotales()
	{
		$q = "SELECT isnull(count(*),0) as total_unidades
			FROM
			(
				SELECT 	
				ex.ID_Solicitud
				FROM 
					[IHTT_DB].[dbo].[TB_Expedientes] ex
				JOIN 
					[IHTT_DB].[dbo].[TB_Solicitud_Vehiculo_Actual] veh on veh.ID_Solicitud = ex.ID_Solicitud
				WHERE ex.Preforma =  :ID_Solicitud
				UNION ALL
				SELECT 	
				ex.ID_Solicitud
				FROM 
					[IHTT_DB].[dbo].[TB_Expedientes] ex
				JOIN 
					[IHTT_DB].[dbo].[TB_Solicitud_Vehiculo_Entra] veh on veh.ID_Solicitud = ex.ID_Solicitud
				WHERE ex.Preforma =  :ID_Solicitud1
			) AS result";
		if (!isset($_POST["echo"])) {
			return $this->select($q, array(':ID_Solicitud' => $_POST["RAM"],':ID_Solicitud1' => $_POST["RAM"]));
		} else {
			echo json_encode($this->select($q, array(':ID_Formulario_Solicitud' => $_POST["RAM"])));
		}
	}	

	protected function getUnidadesEXP($TRAMITES)
	{
		//*****************************************************************************************************/
		//* Construir la condicion para el IN de las concesiones
		//*****************************************************************************************************/
		...
		//* Dependiendo si son certificados o permisos especiales se arma la condicion
		//*****************************************************************************************************/
		$condicion = "Certificado_Operacion IN " . $concesiones;
		//*****************************************************************************************************/
		//* Armando SQL de Unidades
		//*****************************************************************************************************/
		$q = "WITH Unidades AS (
			SELECT 
				-- Certificado unificado (toma el primero no vacío)
				COALESCE(NULLIF(RTRIM(ex.[Certificado_Operacion]),''),
						NULLIF(RTRIM(ex.[N_Permiso_Especial]),''),
						NULLIF(RTRIM(veh.[Numero_Certificado]),''),
						NULLIF(RTRIM(veh.[Numero_PermisoEspecial]),''),
						'SIN-CONCESION') AS Certificado_Operacion,
				vl.[Clase Servicio],
				CASE 
					WHEN vl.[Clase Servicio] IN ('STEC', 'STPC') THEN 
						(
							SELECT tt.DESC_Tipo_Vehiculo 
							FROM [IHTT_SGCERP].[dbo].[TB_Vehiculo_Transporte_Carga] vv
							JOIN [IHTT_SGCERP].[dbo].[TB_Vehiculo_Transporte_Carga_x_Placa] pp 
								ON vv.ID_Vehiculo_Carga = pp.ID_Vehiculo_Carga
							JOIN [IHTT_SGCERP].[dbo].[TB_Tipo_Vehiculo_Transporte_Carga] tt 
								ON vv.ID_Tipo_Vehiculo_Carga = tt.ID_Tipo_Vehiculo_Carga
							WHERE pp.Estado = 'ACTIVA' AND vv.ID_Vehiculo_Carga = vl.ID_Vehiculo
						)
					ELSE 
						(
							SELECT tt.DESC_Tipo_Vehiculo_Transporte_Pas 
							FROM [IHTT_SGCERP].[dbo].[TB_Vehiculo_Transporte_Pasajero] vv
							JOIN [IHTT_SGCERP].[dbo].[TB_Vehiculo_Transporte_Pasajero_x_Placa] pp 
								ON vv.ID_Tipo_Vehiculo_Transporte_Pas = pp.ID_Vehiculo_Transporte
							JOIN [IHTT_SGCERP].[dbo].[TB_Tipo_Vehiculo_Transporte_Pasajero] tt 
								ON vv.ID_Tipo_Vehiculo_Transporte_Pas = tt.ID_Tipo_Vehiculo_Transporte_Pas
							WHERE pp.Estado = 'ACTIVA' AND vv.ID_Vehiculo_Transporte = vl.ID_Vehiculo
						)
				END AS DESC_Tipo_Vehiculo,
			...
			FROM [IHTT_DB].[dbo].[TB_Expedientes] ex
			JOIN [IHTT_DB].[dbo].[TB_Solicitud_Vehiculo_Actual] veh ON veh.ID_Solicitud = ex.ID_Solicitud
			JOIN [IHTT_SGCERP].[dbo].[TB_Marca_Vehiculo] mar ON veh.ID_Marca = mar.ID_Marca
			JOIN [IHTT_SGCERP].[dbo].[TB_Color_Vehiculos] col ON veh.ID_Color = col.ID_Color
			JOIN [IHTT_SGCERP].[dbo].[v_Listado_General] vl 
				ON vl.N_Certificado IN (
						COALESCE(NULLIF(ex.Certificado_Operacion,''), ''),
						COALESCE(NULLIF(ex.N_Permiso_Especial,''), ''),
						COALESCE(NULLIF(veh.Numero_Certificado,''), ''),
						COALESCE(NULLIF(veh.Numero_PermisoEspecial,''), '')
					)
			WHERE ex.Preforma = :ID_Solicitud
			...
			FROM [IHTT_DB].[dbo].[TB_Expedientes] ex
			JOIN [IHTT_DB].[dbo].[TB_Solicitud_Vehiculo_Entra] veh ON veh.ID_Solicitud = ex.ID_Solicitud
			JOIN [IHTT_SGCERP].[dbo].[TB_Marca_Vehiculo] mar ON veh.ID_Marca = mar.ID_Marca
			JOIN [IHTT_SGCERP].[dbo].[TB_Color_Vehiculos] col ON veh.ID_Color = col.ID_Color
			JOIN [IHTT_SGCERP].[dbo].[v_Listado_General] vl 
				ON vl.N_Certificado IN (
		...
		select * from Unidades where " . $condicion . " ORDER BY ESTADO,Certificado_Operacion;";
		if (!isset($_POST["echo"])) {
	...
	protected function validarAccesoEstadoInicial()
	{
		$p = array(":usuario" => $_SESSION['user_name'],":ID_Estado" => $this->appcfg_estado_inicial);
		$q = "SELECT ID FROM [IHTT_RENOVACIONES_AUTOMATICAS].[DBO].[TB_Estados_User] WHERE usuario =:usuario and [ID_Estado] = :ID_Estado and [estado] = 1";
		if (!isset($_POST["echo"])) {
			return $this->select($q, $p);
		} else {
			echo json_encode($this->select($q, $p));
		}
	}
	...
	protected function getDatosPorOmisionPaginacion()
	{
		//*************************************************************************************/
		//* Si es un FSL que ya estaba salvado y se va a continuar trabajando
		//*************************************************************************************/
		if (isset($_POST['RAM']) && $_POST['RAM'] !== '' && $_POST['RAM'] !== null) {
			$SolictanteBasEXP = $this->getSolicitanteBasEXP(); // (respeta tu nombre de método/var)
			if (!isset($SolictanteBasEXP[0])) {
				// ---- RAM (preforma) ----
				$datos[4] = $this->getSolicitanteRAM();
				if (isset($datos[4][0]) && $datos[4][0] !== false) {
					$datos[5]  = $this->getTramitesRAM();
					if (is_array($datos[5]) and count($datos[5]) > 0) {
						if (isset($_POST["page"]) && (int) $_POST["page"] == 1) {
							$datos['Reportes'] = $this->getLinkReportes($datos[4],$datos[5],false);
						}				
						$datos[7]  = $this->getUnidades($datos[5]);						
					}
					$datos[99] = 'RAM';
					...
					echo json_encode([
						"error"     => 90001,
						"errorhead" => 'RAM <strong>' . $_POST['RAM'] . '</strong> NO ENCONTRADA',
						"errormsg"  => 'SI LA RAM NO PUDO SER RECUPERADA PUEDE DEBERSE A QUE NO TIENE ACCESO AL ESTADO ACTUAL O QUE EL NÚMERO DE RAM NO ES CORRECTO. SI PERSISTE, CONTACTE AL ADMINISTRADOR DEL SISTEMA'
				...
				if (isset($datos[4][0]) && $datos[4][0] !== false) {
					...
					if (is_array($datos[5]) and count($datos[5]) > 0) {
						if (isset($_POST["page"]) && (int) $_POST["page"] == 1) {
							$datos['Reportes'] = $this->getLinkReportes($datos[4],$datos[5],true);
						}							
						$datos[7]  = $this->getUnidadesExp($datos[5]);						
					}
					$datos[12] = $_POST['RAM'];
					$datos[99] = 'EXP';
					echo json_encode($datos, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_INVALID_UTF8_SUBSTITUTE);
					flush(); // empuja el buffer al servidor web
					exit;    
				} else {
					echo json_encode([
						"error"     => 90002,
						"errorhead" => 'EXPEDIENTE <strong>' . $_POST['RAM'] . '</strong> NO ENCONTRADO',
						"errormsg"  => 'SI EL EXPEDIENTE NO PUDO SER RECUPERADO PUEDE DEBERSE A QUE NO TIENE ACCESO AL ESTADO ACTUAL O QUE EL NÚMERO DE EXPEDIENTE NO ES CORRECTO. SI PERSISTE, CONTACTE AL ADMINISTRADOR DEL SISTEMA'
					], JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
					return;
				}
			}
			...
			//************************************************************************************************/
			//* Validando si el usuario tiene acceso al estado inicial de ingreso de RAMs (IDE-7)
			//************************************************************************************************/
			$estadoInicial = $this->validarAccesoEstadoInicial();
			if (isset($estadoInicial[0])) {
				// Autorizado
				$datos[15] = [
					'Estado'      => $this->appcfg_estado_inicial,
					'Desc_Estado' => $this->appcfg_estado_inicial_descripcion
				];
				echo json_encode($datos, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
				return;
			} else {
				...
				echo json_encode([
					"error"     => 1003,
					"errorhead" => 'SIN AUTORIZACIÓN',
					"errormsg"  => 'SU USUARIO NO TIENE ACCESO AL ESTADO INICIAL DE INGRESO DE RAMs.</br>ESTADO INICIAL: <strong>' .
								$this->appcfg_estado_inicial . ' => ' . $this->appcfg_estado_inicial_descripcion . '</strong>'
				], JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
				return;
			}
		}
		...
		if ($datos[1] != false && $datos[2] != false) {			
			...
			if ($_POST["RAM"] != '' && $_POST["RAM"] != Null) {
				...
				if (!isset($SolictanteBasEXP[0])) {
					...
					if (isset($datos[4][0]) and $datos[4][0] != false) {
						$datos[3] = $this->getApoderadoLegalRAM();
						$datos[13] = $this->getTramitesEXPTotales();
						$datos[6] = $this->getDocumentos();				
						$datos[14] = $this->getUnidadesExpTotales();
						$datos[8] = $this->testFileExists();
			...
			echo json_encode(array("error" => 1001, "errorhead" => 'UBICACION DE ENTREGA', "errormsg" => 'ALGO RARO SUCEDIO RECUPERANDO LOS DATOS DE UBICACIONES Y DEPARTAMENTOS, INTENTELO DE NUEVO. SI EL PERSISTE CONTACTE AL ADMINISTRADOR DEL SISTEMA'), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
		}
	}
	...
	protected function getUnidadExpediente($RAM, $idConcesion)
	{
		$q = "SELECT 
			...
			(
				...
				WHERE veh.ID_Solicitud = :ID_Solicitud and  ((veh.[Numero_PermisoEspecial] != '' and veh.[Numero_PermisoEspecial] = :Concesion) or
				 (veh.[Numero_Certificado] != '' and veh.[Numero_Certificado] = :Concesion1))
				UNION
				SELECT 	
					CASE 
						WHEN veh.[Numero_Explotacion] != '' THEN RTRIM(veh.[Numero_Certificado])
				...
				JOIN
					[IHTT_SGCERP].[dbo].[v_Listado_General] vl ON vl.N_Certificado = 
						...
						WHERE soli.ID_Solicitud = veh.ID_Solicitud)
				WHERE veh.ID_Solicitud = :ID_Solicitud1 and ((veh.[Numero_PermisoEspecial] != '' and veh.[Numero_PermisoEspecial] = :Concesion2) or
				 (veh.[Numero_Certificado] != '' and veh.[Numero_Certificado] = :Concesion3))
			) AS result
			ORDER BY 
				ESTADO,
				Certificado_Operacion;";
				if (!isset($_POST["echo"])) {
			return $this->select($q, array(":ID_Solicitud" => $RAM, ":ID_Solicitud1" => $RAM, ":Concesion" => $idConcesion,":Concesion1" => $idConcesion,":Concesion2" => $idConcesion,":Concesion3" => $idConcesion));
		} else {
			echo json_encode($this->select($q, array()));
		}
	}
	...
	protected function getAvisodeCobroxPlaca($Numero_Placa,$Numero_Placa_Anterior)
	{
		$row_rs_stmt['error'] = false;
		$respuesta[0]['errorcode'] = '';
		try {
			if (in_array($Numero_Placa_Anterior, $this->appcfg_placas)) {
				...
				$stmt->execute(array(':Numero_Placa' => $Numero_Placa_Anterior,':Numero_Placa1' => '%' . $Numero_Placa_Anterior . '%'));
				...
				$query_rs_stmt = "SELECT [Numero_Placa],[MONTO]
				FROM [IHTT_Webservice].[dbo].[TB_AvisoCobroEnc] ENC,[IHTT_Webservice].[dbo].[TB_AvisoCobroDET] DET, [IHTT_DB].[dbo].[TB_Tramite] TR
				WHERE ENC.AvisoCobroEstado = 2 AND ENC.CodigoAvisoCobro = DET.CodigoAvisoCobro AND DET.CodigoTipoTramite = TR.[ID_Tramite] AND TR.[ID_Tipo_Tramite] = 'IHTTTRA-03' AND (TR.[ID_Clase_Tramite] = 'CLATRA-15' OR TR.[ID_Clase_Tramite] = 'CLATRA-08') AND ([Numero_Placa] = :Numero_Placa OR DET.DescripcionDetalle like :Numero_Placa1 or [Numero_Placa] = :Numero_Placa2 OR DET.DescripcionDetalle like :Numero_Placa3)";
				// Recueprando la información del stmt
				$stmt = $this->db->prepare($query_rs_stmt);
				$stmt->execute(array(':Numero_Placa' => $Numero_Placa_Anterior,':Numero_Placa1' => '%' . $Numero_Placa_Anterior . '%',':Numero_Placa2' => $Numero_Placa,':Numero_Placa3' => '%' . $Numero_Placa . '%'));
				$row_rs_stmt = $stmt->fetch();
				$res = $stmt->errorInfo();
				if (isset($res) and isset($res[3]) and intval(Trim($res[3])) <> 0) {
					$row_rs_stmt['error'] = true;
			...
			if (is_array($rows) && count($rows) > 0) {
				...
				foreach ($rows as $row) {
					if ($row['Acronimo_Clase'] ==  'CU' || $row['Acronimo_Clase'] ==  'CL') {
						...
						. '<input data-validar-salvar="false" style="display:none; text-transform: uppercase;" 
	...
	//* Inicio: Recuperando Tramites Preforma
	//*****************************************************************************************************/
	protected function getTipoTramiteyClaseTramiteExpediente($filtro = array(), $ID_Categoria, $RAM, $idConcesion)
	{
		$joined_string = "'" . implode("', '", $filtro) . "'";
		$q =  "SELECT SOL.ID,
    CONCAT(T.ID_Tipo_Tramite, '_', C.ID_Clase_Tramite, '_', T.Acronimo_Tramite, '_', C.Acronimo_Clase) AS ID_CHECK,
    ...
    END AS Checked
		...
		ON 
			...
			AND TR.ID_Clase_Tramite = C.ID_Clase_Tramite
		LEFT OUTER JOIN 
			[IHTT_DB].[dbo].[TB_Expediente_X_Tipo_Tramite] SOL 
		ON 
			TR.ID_Tramite = SOL.ID_Tramite and SOL.ID_Solicitud = :ID_Formulario_Solicitud and (SOL.Certificado_Operacion = :N_Certificado or SOL.N_Permiso_Especial = :N_Permiso_Especial)
		WHERE 
			TR.ID_Categoria = :ID_Categoria
			AND T.Es_Renovacion_Automatica = 1 
			AND C.ID_Tipo_Tramite IS NOT NULL 
		...
		if ($bandera == 1) {
			...
			foreach ($rows as $row) {
				if ($row['Acronimo_Clase'] ==  'CU' || $row['Acronimo_Clase'] ==  'CL') {
					$display = 'display:' .($row['Checked'] == 'checked=true') ? 'flex;' : 'none;';
					$html = $html . '<div class="row border border-info" id="row_tramite_' . $row['Acronimo_Tramite'] . '_' . $row['Acronimo_Clase'] . '">
					<div class="col-md-1">
					  <input ' . $row['Checked'] . ' data-iddb="' . $row['ID'] . '" data-monto="' . $row['Monto'] . '" 
							 class="form-check-input tramiteschk" 
	...
	protected function getColorByDesc($DescColor)
	{
		...
		$query_rs_color = "SELECT ID_Color,DESC_Color FROM [IHTT_SGCERP].[dbo].[TB_Color_Vehiculos] where DESC_Color = :DESC_Color ORDER BY DESC_Color";
		try {
	...
	protected function getMarcaByDesc($DescMarca)
	{
		...
		$query_rs_Marca = "SELECT ID_Marca,DESC_Marca FROM [IHTT_SGCERP].[dbo].[TB_Marca_Vehiculo] where DESC_Marca = :DESC_Marca ORDER BY DESC_Marca";
		try {
	...
	protected function getDatosUnidadDesdeIP($placa)
	{
		...
		if (isset($vehiculo->codigo) == true && $vehiculo->codigo == 200) {
			...
			if (trim($vehiculo->cargaUtil->vin) == '') {
				$vehiculo->cargaUtil->vin = 'NOAPLICA';
			}
			if (trim($vehiculo->cargaUtil->motor) == '') {						
				$vehiculo->cargaUtil->motor = 'NOAPLICA';
			}


			$comb = strtoupper(trim($vehiculo->cargaUtil->combustible ?? ''));
			// Limpia caracteres invisibles y normaliza el texto
			$comb = preg_replace('/[^A-Z0-9\/]/', '', $comb); 
			if ($comb == '' || $comb == 'NOUTILIZA' || $comb == 'N/A' || $comb == 'NA') {
				$vehiculo->cargaUtil->combustible = 'NOAPLICA';
			}						
			...
			if ($_POST["action"] == "get-vehiculo") {
				...
				$vehiculo->cargaUtil->Expedientes = $this->validarExpedientes($vehiculo->cargaUtil->placa, $vehiculo->cargaUtil->placaAnterior, $_POST["Concesion"]);
			}
		} else {
	...
	protected function procesarFechaDeVencimiento($record, $id_clase_servico)
	{
		...
		if (isset($record["Fecha_Expiracion"])) {
			...
			//*************************************************************************************/
			//* Limitando la cantidad de periodos a cobrar
			//* Variable de configuración appcfg_max_periodos_anterior_cobrar			
			//*************************************************************************************/
			if (intval($this->appcfg_max_periodos_anterior_cobrar) > 0 and intval($contadorconcesion) > intval($this->appcfg_max_periodos_anterior_cobrar)){
				$contadorconcesion = intval($this->appcfg_max_periodos_anterior_cobrar);
			}
			...
			if (isset($record["Fecha_Expiracion_Explotacion"])) {
				...
				//* Variable de configuración appcfg_max_periodos_anterior_cobrar			
				//*************************************************************************************/
				if (intval($this->appcfg_max_periodos_anterior_cobrar) > 0 and intval($contadorpermisoexplotacion) > intval($this->appcfg_max_periodos_anterior_cobrar)){
					$contadorpermisoexplotacion = intval($this->appcfg_max_periodos_anterior_cobrar);
				}
	...
	protected function getDatosMulta($placa, $placa_anterior,$rtn_concesionario='false',$rtn_propietario='false'): mixed
	{
		...
		if ($rtn_concesionario != 'false' or $rtn_propietario != 'false') {
			if (trim($rtn_concesionario) != trim($rtn_propietario)) {
				...
				$parametros = array(":Placa_Actual" => $placa, ":Placa_Anterior" => $placa_anterior,":Identidad_RTN" => substr($rtn_concesionario,0,13), ":Identidad_RTN1" => substr($rtn_propietario,0,13), ":RTNConcesionario" => $rtn_propietario, ":RTNConcesionario1" => $rtn_propietario);
			} else {
				$query = "SELECT MUL.Multa,Avi.CodigoAvisoCobro,MUL.Fecha,MUL.Propietario,MUL.Identidad_RTN,MUL.Certificado,MUL.Placa,convert(numeric(10,2), MUL.Total) as Total 
				FROM [IHTT_MULTAS].[dbo].[V_Multas_IHTT_DGT] MUL,[IHTT_Webservice].[dbo].[TB_AvisoCobroEnc] Avi
				left outer join [IHTT_Webservice].[dbo].[TB_ArregloEnc] Enc on Avi.CodigoAvisoCobro  = Enc.ID_Aviso
				WHERE MUL.ID_Estado='1' AND MUL.Multa = avi.ID_Solicitud and 
				(MUL.Placa= :Placa_Actual or MUL.Placa= :Placa_Anterior or SUBSTRING(REPLACE(REPLACE(Mul.Identidad_RTN,' ',''),'-',''),1,13) = :Identidad_RTN or Avi.RTNConcesionario = :RTNConcesionario) and
				(
				(
				select count(*) from [IHTT_Webservice].[dbo].[TB_AvisoCobroEnc] Encx where Encx.AvisoCobroEstado = 1 and
				Encx.ID_Solicitud  = Enc.Numero_Arreglo
				...
				$parametros = array(":Placa_Actual" => $placa, ":Placa_Anterior" => $placa_anterior,":Identidad_RTN" => substr($rtn_concesionario,0,13), ":RTNConcesionario" => $rtn_propietario);
			}
		} else {	
			$query = "SELECT MUL.Multa,Avi.CodigoAvisoCobro,MUL.Fecha,MUL.Propietario,MUL.Identidad_RTN,MUL.Certificado,MUL.Placa,convert(numeric(10,2), MUL.Total) as Total 
			FROM [IHTT_MULTAS].[dbo].[V_Multas_IHTT_DGT] MUL,[IHTT_Webservice].[dbo].[TB_AvisoCobroEnc] Avi
	...
	protected function getConcesion()
	{
		...
		if ($datos[0] > 0 and $data[0]['RTN_Concesionario'] == $_POST["RTN_Concesionario"]) {
				...
				if ($data && $data[0] && $data[0]["Unidad"] && $data[0]["Unidad"][0]['ID_Placa']) {
					...
					if (isset($vehiculo->codigo) == true && $vehiculo->codigo == 200) {
						...
						if ($vehiculo->cargaUtil->placa != $vehiculo->cargaUtil->placaAnterior) {
							...
							$row_rs_stmt = $this->getAvisodeCobroxPlaca($vehiculo->cargaUtil->placa,$vehiculo->cargaUtil->placaAnterior);
			...
			if ($data[0]["Tramites"]===false) {
				$data = (array("error" => 4009, "errorhead" => 'TRAMITES SIN TARIFA', "errormsg" => "LA CONCESION NO. " . $_POST["Concesion"] . " NO TIENE TARIFAS ASIGNADAS SEGUN SU CATEGRIA: " . $data[0]["Categoria"] . ", FAVOR CONTACTE A LA UNIDAD DE INFOTECNOLOGIA"));
			}
		} else {
			if ($datos[0] > 0) {
	...
	//*************************************************************************************/
	//* FUNCION PARA RECUPERAR LA CONCESION EN PREFORMA
	//*************************************************************************************/
		...
		if ($datos[0] > 0) {
			...
			if ($data[0]["Clase Servicio"] == 'STEC') {
				...
				if (
					...
					$row_rs_stmt = $this->getAvisodeCobroxPlaca(Trim($data[0]["Unidad"][0]['ID_Placa']),Trim($data[0]["Unidad"][0]['ID_Placa_Antes_Replaqueo']));
					if (isset($row_rs_stmt['MONTO'])) {
						$data[0]["Unidad"][0]['estaPagadoElCambiodePlaca'] = true;
					} else {
						$data[0]["Unidad"][0]['estaPagadoElCambiodePlaca'] = false;
				...
				if (isset($data[0]["Unidad"]) and isset($data[0]["Unidad"][0]) and isset($data[0]["Unidad"][0]['ID_Placa'])) {
					$data[0]["Unidad"][0]['Multas'] = $this->getDatosMulta($data[0]["Unidad"][0]['ID_Placa'], $data[0]["Unidad"][0]['ID_Placa_Antes_Replaqueo'],$data[0]['RTN_Concesionario'],$data[0]["Unidad"][0]['RTN_Propietario']);
				}
	...
	//* FUNCION PARA RECUPERAR LA CONCESION EN PREFORMA
	//*************************************************************************************/
	protected function getConcesionExpediente()
	{
		...
		if ($datos[0] > 0) {
			...
			if ($data[0]["Clase Servicio"] == 'STEC') {
				$data[0]["Link"] = $_SESSION["appcfg_Dominio_Raiz"] . ":172/api_rep.php?action=get-PDFPermisoEsp-Carga&PermisoEspecial=" . $data[0]["CertificadoEncriptado"];
				$data[0]["Vista"] = file_get_contents("vistas/pes_carga.html");
				$data[0]["Unidad"] = $this->getUnidadExpediente($_POST["RAM"], $_POST["idConcesion"]);
				//**********************************************************************************************************************/
				//* Recuperando Bandera Sobre Si Pago el Cambio de Placa o No
				//**********************************************************************************************************************/
				if (
				...
				if ($data[0]["Clase Servicio"] == 'STEP') {
					...
					if ($data[0]["Clase Servicio"] == 'STPC') {
						...
						$data[0]["Tipo_Concesion"] = 'CERTIFICADO DE OPERACIÓN:';
						$data[0]["Tramites"] = $this->getTipoTramiteyClaseTramiteExpediente(['PE',  'CO', 'CL', 'CM', 'CC', 'CS', 'CU'], $data[0]["Categoria"], $_POST["RAM"], $_POST["idConcesion"]);
						$tipo = $this->getCategoriaEspecilizadaCarga($data[0]["Id_Tipo_Categoria"]);
						if (count($tipo) > 0) {
							$data[0]["Tipo_Categoria_Especilizada"] = $tipo[0]['value'];
							$data[0]["Desc_Categoria_Especilizada"] = $tipo[0]['text'];
	...
	protected function validarEnExpediente($ID_Placa, $ID_Placa_Antes_Replaqueo, $Concesion, $RAM=''): mixed{
		return array();
	}
	//**************************************************************************************/
	...
	//**************************************************************************************/
	protected function validarEnPreforma($ID_Placa, $ID_Placa_Antes_Replaqueo, $Concesion, $RAM='',$MODALIDAD_INGRESO='I'): mixed
	{
		//**************************************************************************************/
	...
	protected function validarExpedientes(string $placa, string $placa1, string $concesion): mixed
	{
		...
		resueltos AS (
			...
			FROM IHTT_DB.dbo.TB_Expediente_Acumulado AS A inner join IHTT_GDL.dbo.TB_Resolucion r on A.ID_Solicitud_Primario = r.ID_Solicitud
		)
		SELECT
				c.ID_Expediente,
		...
		AND (
			(LTRIM(RTRIM(c.N_Permiso_Especial)) <> '' AND LTRIM(RTRIM(c.N_Permiso_Especial)) = :Concesion) or
			(LTRIM(RTRIM(c.Certificado_Operacion)) <> '' AND LTRIM(RTRIM(c.Certificado_Operacion)) = :Concesion1)
			OR (
			EXISTS (
				SELECT 1
				FROM IHTT_DB.dbo.TB_Expediente_X_Tipo_Tramite AS t
				WHERE t.ID_Solicitud = c.ID_Solicitud AND 
				(
				(LTRIM(RTRIM(t.N_Permiso_Especial)) <> '' and isnull(LTRIM(RTRIM(t.N_Permiso_Especial)),'') = :Concesion2) or
				(LTRIM(RTRIM(t.Certificado_Operacion)) <> '' and isnull(LTRIM(RTRIM(t.Certificado_Operacion)),'') =  :Concesion3)
				)
			)
			)
		)";
		$parametros = array(":Concesion" => $concesion,":Concesion1" => $concesion,":Concesion2" => $concesion,":Concesion3" => $concesion);
		$row = $this->select($query, $parametros);
		if ($row != false and count($row) > 0) {
			$titulos = [
				...
				2 => 'CONCESION',
				3 => 'RTN SOLICITANTE',
				4 => 'NOMBRE SOLICITANTE',
				5 => 'SISTEMA FECHA',
				6 => 'ESTADO',
				7 => 'PERMISO EXPLOTACION',
				'ID_Expediente' => 'ID EXPEDIENTE',
				'ID_Solicitud'  => 'ID SOLICITUD',
				'CONCESION' => 'CONCESION',
				'ID_Solicitante' => 'RTN SOLICITANTE',
				'NombreSolicitante' => 'NOMBRE SOLICITANTE',
				'SitemaFecha' => 'SISTEMA FECHA',
				'Expediente_Estado' => 'ESTADO',
				'Permiso_Explotacion' => 'PERMISO EXPLOTACION'
			];
			$row[count($row) + 1] = $titulos;
		}
		return $row;
	...
	protected function saveSolicitante($Concesion, $Apoderado, $Solicitante, $row_ciudad, $RAM,$UsuarioAsigna)
	{
		...
		$parametros = array(
			...
			":Nombre_Solicitante" => substr(strtoupper($Solicitante['Nombre']),0,300),
			":ID_Tipo_Solicitante" => $Solicitante['Tipo_Solicitante'],
			":RTN_Solicitante" => $Solicitante['RTN'],
			":Domicilo_Solicitante" => substr(strtoupper($Solicitante['Domicilio']),0,500),
			":Denominacion_Social" => substr(strtoupper($Solicitante['Denominacion']),0,300),
			":ID_Aldea" => $Solicitante['Aldea'],
			":Telefono_Solicitante" => substr($Solicitante['Telefono'],0,9),
			...
			":Etapa_Preforma" => 1,
			":Usuario_Acepta" => $UsuarioAsigna[0]["Nombre_Usuario"], 
			":Codigo_Usuario_Acepta" => $UsuarioAsigna[0]["Codigo_Usuario"],
	...
	protected function saveUnidadExpediente($RAM, $Unidad, $Concesion, $tabla)
	{
		...
		$parametros = array(
			...
			":Capacidad_Carga" => floatval($Unidad['Capacidad']),
			":Peso_Unidad" => 0,
			":Numero_Explotacion" => strtoupper($PERMISO_EXPLOTACION),
			":Numero_Certificado" => strtoupper($CERTIFICADO_OPERACION),
			":Numero_PermisoEspecial" => strtoupper(string: $PERMISO_ESPECIAL),
			":ID_Placa_Antes_Replaqueo" => strtoupper($Unidad['ID_Placa_Antes_Replaqueo']),
			":Sistema_Usuario" => $_SESSION["user_name"]
		);
		return $this->insert($query, $parametros);
	...
	protected function saveTramitesExpediente($RAM, $Tramites)
	{
		if (isset($_POST["echo"])) {
			$this->db->beginTransaction();
		}
	...
	protected function updateSolicitanteExpediente($Solicitante)
	{	
		$query = "UPDATE [IHTT_DB].[dbo].[TB_Solicitante]
		SET  
		 [NombreSolicitante] = :NombreSolicitante
		,[NombreEmpresa] = :NombreEmpresa
		,[RTNSolicitante] = :RTNSolicitante
		,[CodigoSolicitanteTipo] = :CodigoSolicitanteTipo
		...
		$parametros = array(
			":NombreSolicitante" => strtoupper($Solicitante['Nombre']),
			":NombreEmpresa" => strtoupper($Solicitante['Denominacion']),
			":RTNSolicitante" => $Solicitante['RTN'],
			":CodigoSolicitanteTipo" => $Solicitante['Tipo_Solicitante'],			
			":IDSolicitante" => substr($Solicitante['RTN'],0,13),			
			":Direccion" => strtoupper($Solicitante['Domicilio']),
			":Telefono" => $Solicitante['Telefono'],
			":Email" => $Solicitante['Email'],
			":Aldea" => $Solicitante['Aldea'],
			":SistemaUsuario" => $_SESSION['user_name'],
			":ID_Solicitante" => $Solicitante['RTN']);
		return $this->update($query, $parametros); 
	}
	...
	protected function updateApoderado($RAM, $Apoderado)
	{
		...
		$parametros = array(
			":Nombre_Apoderado_Legal" => strtoupper($Apoderado['Nombre']),
			":Ident_Apoderado_Legal" => substr($Apoderado['RTN'],0,13),
			":Direccion_Apoderado_Legal" => strtoupper($Apoderado['Direccion']),
			":Telefono_Apoderado_Legal" => $Apoderado['Telefono'],
			":Email_Apoderado_Legal" => strtoupper($Apoderado['Email']),
			":ID_Formulario_Solicitud" => $RAM,
	...
	protected function updateApoderadoExpediente($Apoderado)
	{
		$query = "UPDATE [IHTT_DB].[dbo].[TB_Apoderado_Legal]
		SET 
			Nombre_Apoderado_Legal = :Nombre_Apoderado_Legal,
			Identidad = :Identidad,
			Direccion = :Direccion,
		...
		$parametros = array(
			":Nombre_Apoderado_Legal" => strtoupper($Apoderado['Nombre']),
			":Identidad" => substr($Apoderado['RTN'],0,13),
			":Direccion" => strtoupper($Apoderado['Direccion']),
			":Telefono" => $Apoderado['Telefono'],
			":Email" => strtoupper($Apoderado['Email']),
			":SistemaUsuario" => $_SESSION['user_name'],
	...
	protected function updateUnidadExpediente($Unidad, $Concesion, $tabla)
	{
		...
		//* Consulta SQL para actualizar el vehículo
		$query = "UPDATE " . $tabla . " SET RTN_Propietario = :RTN_Propietario,
			Nombre_Propietario = :Nombre_Propietario,
			ID_Placa = :ID_Placa,
			ID_Marca = :ID_Marca,
			Anio = :Anio,
			...
			Capacidad_Carga = :Capacidad_Carga,
			Peso_Unidad = 0,  
			Numero_Explotacion = :Permiso_Explotacion,
			Numero_Certificado = :Certificado_Operacion,
			Numero_PermisoEspecial = :Permiso_Especial,
			ID_Placa_Antes_Replaqueo = :ID_Placa_Antes_Replaqueo,
			Sistema_Fecha = SYSDATETIME(),
			Sistema_Usuario = :Sistema_Usuario
		WHERE 
	...
	protected function updateExpediente()
	{
		...
		if ($_POST["Concesion"]['esCambioDeVehiculo'] == true) {
			...
			$responseValidarMultas1 = $this->getDatosMulta($_POST["Unidad1"]['Placa'], $_POST["Unidad1"]['ID_Placa_Antes_Replaqueo'],$_POST["Solicitante"]['RTN'],$_POST["Unidad"]['RTN_Propietario']);
			...
			$responseValidarExpedientes = $this->validarExpedientes($_POST["Unidad1"]['Placa'], $_POST["Unidad1"]['ID_Placa_Antes_Replaqueo'], $_POST["Concesion"]['Certificado']==''?$_POST["Concesion"]['Permiso_Especial']:$_POST["Concesion"]['Certificado']);
		} else {
			...
			$responseValidarExpedientes = $this->validarExpedientes($_POST["Unidad"]['Placa'], $_POST["Unidad"]['ID_Placa_Antes_Replaqueo'], $_POST["Concesion"]['Certificado']==''?$_POST["Concesion"]['Permiso_Especial']:$_POST["Concesion"]['Certificado']);
		}
		...
		if (
			...
			(((isset($responseValidarExpedientes)   and $responseValidarExpedientes    == false and is_array($responseValidarExpedientes) == false)))   or
			...
			echo json_encode([
				...
				'Expedientes' =>  $responseValidarExpedientes,
				'Preforma'    =>  isset($responseValidarPreforma) ? $responseValidarPreforma : ''
			]);
		} else {
			$isOKUnidad = $this->updateUnidadExpediente( $_POST["Unidad"], $_POST["Concesion"],'TB_Solicitud_Vehiculo_Actual');
			if ($isOKUnidad == false) {
				$this->db->rollBack();
				...
				if ($_POST["Concesion"]['esCambioDeVehiculo'] == true) {
					if ($_POST["Unidad1"]['ID_Unidad'] !== '' && $_POST["Unidad1"]['ID_Unidad'] !== null) {
						$isOKUnidad1 = $this->updateUnidadExpediente( $_POST["Unidad1"], $_POST["Concesion"], 'TB_Solicitud_Vehiculo_Entra');
					} else {
						$isOKUnidad1 = $this->saveUnidadExpediente($RAM['nuevo_numero'], $_POST["Unidad1"], $_POST["Concesion"], 'TB_Solicitud_Vehiculo_Entra');
					}
	...
	//*******************************************************************************************************************/
	protected function saveExpediente()	{
		// BANDERA DE ERROR
		$ERROR = false;
		// Start a transaction
		$this->db->beginTransaction();
		...
		if (
			...
			$isOKSolicitante = $this->updateSolicitanteExpediente( $_POST["Solicitante"]);
			if ($isOKSolicitante == false) {
				...
				$isOKApoderado = $this->updateApoderadoExpediente( $_POST["Apoderado"]);
				if ($isOKApoderado == false) {
					$this->db->rollBack();
					echo json_encode(array("error" => 4001, "errorhead" => "INCONVENIENTES", "errormsg" => 'INTENTANTO SALVAR EL APODERADO'));
				} else {
					$isOKUnidad = $this->saveUnidadExpediente($RAM['nuevo_numero'], $_POST["Unidad"], $_POST["Concesion"], 'TB_Solicitud_Vehiculo_Actual');
					if ($isOKUnidad == false) {
						$this->db->rollBack();
						...
						if ($ERROR == false) {
							$isOKTramites = $this->saveTramitesExpediente($RAM['nuevo_numero'], $_POST["Tramites"]);
							if ($isOKTramites[0] == false) {
								...
								echo json_encode(
									[
										...
										'Estado'         =>  ['Estado' => $this->appcfg_estado_inicial,'Desc_Estado' => $this->appcfg_estado_inicial_descripcion]
									]
								);
							}
						}
	...
	protected function savePreforma()	{
		...
		if (
			...
			if ($_POST["Concesion"]['RAM'] == '') {
				$isOKSolicitante = $this->saveSolicitante($_POST["Concesion"], $_POST["Apoderado"], $_POST["Solicitante"], $responseValidarCiudad, $RAM['nuevo_numero'],$responseValidarUsuario);
			} else {
		...
		try {
			...
			if (isset($res) and isset($res[3]) and intval(trim($res[3])) <> 0) {
				...
				if (isset($record["Fecha_Expiracion"])) {
					...
					//*************************************************************************************/
					//* Si el numero de periodos a cobrar es mayor al maximo se ajusta al maximo
					//*************************************************************************************/
					if (intval($this->appcfg_max_periodos_anterior_cobrar) >  0 and intval($contadorconcesion) > intval($this->appcfg_max_periodos_anterior_cobrar)){
						$contadorconcesion = intval($this->appcfg_max_periodos_anterior_cobrar);
					}
	...
	protected function PDFAvisodeCobroVentanillaApi($rs_id_rs_solicitud,$rs_id_rs_template=5):array {
		...
		foreach ($row_rs_todos_los_registros as $row_rs_expediente){
			...
			if ($row_rs_expediente['ID_Tramite'] != '') {
				...
				$concesionComparar = ($row_rs_expediente['Certificado_Operacion'] != '')? $row_rs_expediente['Certificado_Operacion'] : $row_rs_expediente['N_Permiso_Especial'];
				if ($ConcesionValue != $concesionComparar) {
					$vehiculos = $this->getVehiculosPreforma($rs_id_rs_solicitud,$concesionComparar);
				...
				If (isset($row_rs_Tarifa['Monto'])) {			
					...
					if (isset($concesion['rencon']) and isset($concesion['rencon'][count($concesion['rencon'])]) and isset($concesion['rencon'][count($concesion['rencon'])]['periodo'])){
						$row_rs_expediente['Periodo'] = $concesion['rencon'][count($concesion['rencon'])]['periodo'];
					}
					If ($row_rs_expediente['Permiso_Explotacion'] != '') {
						if (isset($concesion['renperexp']) and is_array($concesion['renperexp']) == true and count($concesion['renperexp']) > 0){
							$row_rs_expediente['Periodo-Explotacion'] = $concesion['renperexp'][count($concesion['renperexp'])]['periodo'];
						}
					}
					//***********************************************************************************/
					...
					if ($esCobroPeriodosAtrasados == true && $row_rs_expediente['ID_Tipo_Tramite'] == 'IHTTTRA-02')          {
						...
						//**********************************************************************************************************************************************/
						if (isset($concesion['rencon']) and is_array($concesion['rencon']) and count($concesion['rencon']) > 1 && ($row_rs_expediente['ID_Clase_Tramite'] == 'CLATRA-02' || $row_rs_expediente['ID_Clase_Tramite'] == 'CLATRA-03'))   {
							$contador_vueltas = 1;                    
							for ($i=count($concesion['rencon'])-1; $concesion['rencon-cantidad']>$contador_vueltas; $i--) {
								$contador++;
								$contador_vueltas++;
								//***********************************************************************************/
						...
						// primer ciclo ya fue procesado anteriormente
						//**********************************************************************************************************************************************/  
							if (isset($concesion['renperexp']) and is_array($concesion['renperexp']) and count($concesion['renperexp']) > 1 && $row_rs_expediente['ID_Clase_Tramite'] == 'CLATRA-01')   {    
								$contador_vueltas = 1;                    
								for ($i=(count($concesion['renperexp'])-1); $concesion['renper-explotacion-cantidad']>$contador_vueltas; $i--) {
									$contador++;
									$contador_vueltas++;
									//***********************************************************************************/
									//*Recuperando el rotulo del periodo a renovar de la concesion
