...
function vista_data(estado = '', descripcion = '', agregar, num) {
   ...
   // if (estadosSistem.includes((estado != '') ? estado : ultimoEstadoSeleccionado)) {
   ...
   document.getElementById("tabla-container").innerHTML = '<center><img width="120px" height="120px" src="' + $appcfg_Dominio_Corto + 'ram/assets/images/hug.gif"></center>';
   ...
   fetch(`${$appcfg_Dominio}/query_tabla_dinamica_general.php?estado=${(estado != '') ? estado : ultimoEstadoSeleccionado}&campo=${campo}&datoBuscar=${datoBuscar}&limit=${rowsPerPage}&page=${(num) ? num : currentPage}&pagados=${pagados}`)
      .then(response => {
         if (!response.ok) {
...
function TablaDinamica(data, descripcion) {
   ...
   if (sonTodosVacios == true) {
      ...
      //*enviando texto
      tableTitle.innerHTML = `<strong>INFORMACIÓN DEL ESTADO ${descripcion}</strong>`;
      ...
      encabezadoExcluye = ['ESTADO','AGREGRAR', 'COMPARTIDO', 'Aviso_Cobro', 'USUARIOS_COMPARTIDOS'];
      ...
      if (descripcion == 'EN PROCESO' || descripcion == 'EN VENTANILLA') {
...
function renderTableRows(data, tbody, todo, descripcion) {
   ...
   data.forEach((fila, index) => {
      ...
      fila.forEach((valor, colIndex) => {
         let activo = (dividirDatosPagos[1] != undefined) ? dividirDatosPagos[1] : 'NO ESTA ACTIVO';
         let folioPago = (dividirDatosPagos[0] != undefined) ? dividirDatosPagos[0] : 'NO HAY PAGO';

         //?nota:Fila[0] es el ram
         let url = `${$appcfg_Dominio}Documentos/${fila[0]}/AvisodeCobro_${fila[0]}.pdf`
         if (colIndex !== 5 && colIndex !== 10 && colIndex !== 11 && colIndex !== 12) {
               ...
               <div style="display: flex; align-items: center; gap: 5px;">
                    ...
                    ${activo === '2'
                     ...
                     : `<span style="display: flex; flex-direction: column; align-items: center; cursor: pointer;">
                           <i class="fas fa-dollar-sign dolar-span text-center"  data-bs-toggle="tooltip" title="NO HAY PAGO" style="font-size: 23px;">
                           </i>
            ...
            `;
               ...
               if (colIndex === 6) {
                  var soloFecha = valor.split(" ")[0];
                   console.log(soloFecha, 'solofecha');
                  td.textContent = soloFecha;
               } else {
                  // if (fila[9] == '1' && colIndex === 0) {
                  //    td.innerHTML = `${valor}  <i class="fas fa-share-alt compartiendo"  data-bs-toggle="tooltip" title="${(fila[11] && fila[11] !== usuario) ? 'PERTENECE A: ' + fila[11].toUpperCase() : ''}" style="font-size: 18px;"></i>`
                  // } else {
            ...
            if (colIndex === 0) {
               ...
               td.onclick = () => {
                  ...
                  if (Consulta == 'SS') {
                     ...
                     window.location.href = `${$appcfg_Dominio}ram.php?RAM=${valor}&Consulta=S`;
                  }
