...
$id_privilegio = 'PRIVILEGIOXUSUARIO';
...
// Armando el query
$query_rs_usuario = "SELECT id, nombre as descripcion from usuarios order by descripcion";
...
if ($rs_tipo_rs_mantenimiento == "") {
	$rs_tipo_rs_mantenimiento = $row_rs_usuario['id'];
}

// Armando el query
$query_rs_privilegio = "SELECT a.icono,a.id, a.descripcion, a.menu_padre from privilegio a where (select count(*) from privilegio_x_usuario b where b.id_usuario = :id_tipo_usuario and a.id = b.id_privilegio) = 0 order by a.nivel_menu";
// Preparando la sentencia
...
// Armando el query
$query_rs_privilegioxusuario = "SELECT a.icono,a.id, a.menu_padre, a.descripcion from privilegio a, privilegio_x_usuario b where a.id = b.id_privilegio and b.id_usuario = :id_tipo_usuario order by a.nivel_menu";
// Preparando la sentencia
...
<div class="table-responsive bg_botones">
...
<div class="table-responsive pantalla_inicio">    
      ...
      <table class="table table-condensed">
        <tr>
          ...
          <td align="left" width="20%">
			  <label data-toggle="tooltip" data-placement="top" title="Hooray!" data-original-title="Seleccione el usuario con el cúal desea trabajar sus privilegios">
          <div id="label-nombre"> Usuario:</div></label></td>
        ...
        <tr>
          <td>&nbsp;</td>
          <td><label class="tooltip-test" data-placement="top" data-original-title="Seleccione los privilegios que desea otorgarle al usuario">Privilegios</label></td>
          <td><label class="tooltip-test" data-placement="top" data-original-title="Aqui apareceran los privilegios que le han sido asignados al usuario">Privilegios por Usuario</label>
</td>
        </tr>
        <tr>
          <td>&nbsp;</td>
          <td>
            ...
            for ($i=0;$totalRows_rs_privilegio>$i;$i++) {
                ...
                <option class="text-info" value="<?php echo $row_rs_privilegio['id'];?>"><?php echo $row_rs_privilegio['id'];?></option>
               <?PHP } else { ?>
               <option class="text-success" value="<?php echo $row_rs_privilegio['id'];?>">&nbsp;&nbsp;&nbsp; <?php echo $row_rs_privilegio['id'];?></option>
               <?PHP }  ?>
                <?PHP 
                $row_rs_privilegio = $privilegio->fetch();          
            } ?>
          ...
          <td>
			...
			<?PHP if($row_rs_privilegioxusuario['menu_padre'] == '' ) { ?>            
	            <option class="text-info" value='<?php echo $row_rs_privilegioxusuario['id'];?>'><?php echo $row_rs_privilegioxusuario['id'];?></option>
            <?PHP } else { ?>
	            <option class="text-success" value='<?php echo $row_rs_privilegioxusuario['id'];?>'>&nbsp;&nbsp;&nbsp;<?php echo $row_rs_privilegioxusuario['id'];?></option>
            <?PHP }  ?>
            <?PHP 
            $row_rs_privilegioxusuario = $privilegioxusuario->fetch();          
            } ?>
...
{
	window.location='privilegiosxusuario.php?id='+f_tipo;
}
</script>
